...
2. **Vector Retrieval Layer**: These summaries are embedded and stored. Active agents query this semantic layer via `semantic_vector_search` dynamically when historical context is required (e.g., "What was our decision on the caching layer in the previous meeting?").

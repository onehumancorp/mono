...
|---------|-----------|-------------|-----------------|--------|
| UT-01 | Role Validation| Hire invalid agent role | Request rejected with 400 | DONE |
| UT-02 | SVID Parse | Parse valid workload SVID | Trust domain and role extracted | DONE |

### 3.2 Integration Tests
| Test ID | Components | Description | Expected Result | Status |
|---------|------------|-------------|-----------------|--------|
| IT-01 | Hub -> SPIRE | Request SVID for new agent | SVID issued < 500ms | DONE |
| IT-02 | Hub -> Gateway | Agent attempts $500 spend tool | Guardian Agent blocks call | DONE |

### 3.3 E2E Tests
| Test ID | CUJ Reference | Description | Expected Result | Status |
|---------|---------------|-------------|-----------------|--------|
| E2E-01 | Hire Agent | Admin clicks "Confirm Hire" | Agent state = IDLE on UI < 100ms | DONE |
| E2E-02 | Approval Gate | Admin swipes "Approve" for spend | Blocked tool call executes | DONE |
| E2E-03 | Cross-Cluster | Hire agent from federated domain | SVID validated, latency < 50ms | DONE |

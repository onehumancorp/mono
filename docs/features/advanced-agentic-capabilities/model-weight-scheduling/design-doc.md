# Design Doc: Model Weight Scheduling
...
The Model Weight Scheduling capability is injected directly into the active LangGraph execution thread.
...
1. **Core Processing Engine**: Evaluates active thread state and applies Model Weight Scheduling logic.
...
- **Context Limit Breaches**: If Model Weight Scheduling payload exceeds max token limits, the Semantic Distillation worker is triggered to summarize context.

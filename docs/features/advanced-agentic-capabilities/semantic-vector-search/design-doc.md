# Design Doc: Semantic Vector Search
...
The Semantic Vector Search capability is injected directly into the active LangGraph execution thread.
...
1. **Core Processing Engine**: Evaluates active thread state and applies Semantic Vector Search logic.
...
- **Context Limit Breaches**: If Semantic Vector Search payload exceeds max token limits, the Semantic Distillation worker is triggered to summarize context.

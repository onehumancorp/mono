# Design Doc: Fail-Closed DNS Verification
...
The Fail-Closed DNS Verification capability is injected directly into the active LangGraph execution thread.
...
1. **Core Processing Engine**: Evaluates active thread state and applies Fail-Closed DNS Verification logic.
...
- **Context Limit Breaches**: If Fail-Closed DNS Verification payload exceeds max token limits, the Semantic Distillation worker is triggered to summarize context.

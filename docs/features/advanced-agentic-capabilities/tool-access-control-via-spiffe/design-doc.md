# Design Doc: Tool Access Control via SPIFFE
...
The Tool Access Control via SPIFFE capability is injected directly into the active LangGraph execution thread.
...
1. **Core Processing Engine**: Evaluates active thread state and applies Tool Access Control via SPIFFE logic.
...
- **Context Limit Breaches**: If Tool Access Control via SPIFFE payload exceeds max token limits, the Semantic Distillation worker is triggered to summarize context.

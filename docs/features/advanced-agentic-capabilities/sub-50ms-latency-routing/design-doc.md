# Design Doc: Sub-50ms Latency Routing
...
The Sub-50ms Latency Routing capability is injected directly into the active LangGraph execution thread.
...
1. **Core Processing Engine**: Evaluates active thread state and applies Sub-50ms Latency Routing logic.
...
- **Context Limit Breaches**: If Sub-50ms Latency Routing payload exceeds max token limits, the Semantic Distillation worker is triggered to summarize context.

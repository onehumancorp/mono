# Design Doc: GraphQL Schema Introspection
...
The GraphQL Schema Introspection capability is injected directly into the active LangGraph execution thread.
...
1. **Core Processing Engine**: Evaluates active thread state and applies GraphQL Schema Introspection logic.
...
- **Context Limit Breaches**: If GraphQL Schema Introspection payload exceeds max token limits, the Semantic Distillation worker is triggered to summarize context.

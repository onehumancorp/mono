# Design Doc: B2B SPIFFE Federation
...
The B2B SPIFFE Federation capability is injected directly into the active LangGraph execution thread.
...
1. **Core Processing Engine**: Evaluates active thread state and applies B2B SPIFFE Federation logic.
...
- **Context Limit Breaches**: If B2B SPIFFE Federation payload exceeds max token limits, the Semantic Distillation worker is triggered to summarize context.

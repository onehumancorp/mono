# Design Doc: Document Layout Extraction
...
The Document Layout Extraction capability is injected directly into the active LangGraph execution thread.
...
1. **Core Processing Engine**: Evaluates active thread state and applies Document Layout Extraction logic.
...
- **Context Limit Breaches**: If Document Layout Extraction payload exceeds max token limits, the Semantic Distillation worker is triggered to summarize context.

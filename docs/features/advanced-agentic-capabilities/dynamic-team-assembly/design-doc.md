# Design Doc: Dynamic Team Assembly
...
The Dynamic Team Assembly capability is injected directly into the active LangGraph execution thread.
...
1. **Core Processing Engine**: Evaluates active thread state and applies Dynamic Team Assembly logic.
...
- **Context Limit Breaches**: If Dynamic Team Assembly payload exceeds max token limits, the Semantic Distillation worker is triggered to summarize context.

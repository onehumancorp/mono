# Design Doc: Memory Forgetting & Decay Logic
...
The Memory Forgetting & Decay Logic capability is injected directly into the active LangGraph execution thread.
...
1. **Core Processing Engine**: Evaluates active thread state and applies Memory Forgetting & Decay Logic logic.
...
- **Context Limit Breaches**: If Memory Forgetting & Decay Logic payload exceeds max token limits, the Semantic Distillation worker is triggered to summarize context.

# Design Doc: Tool Execution Rate Limiting
...
The Tool Execution Rate Limiting capability is injected directly into the active LangGraph execution thread.
...
1. **Core Processing Engine**: Evaluates active thread state and applies Tool Execution Rate Limiting logic.
...
- **Context Limit Breaches**: If Tool Execution Rate Limiting payload exceeds max token limits, the Semantic Distillation worker is triggered to summarize context.

# Design Doc: VRAM Quota Management
...
The VRAM Quota Management capability is injected directly into the active LangGraph execution thread.
...
1. **Core Processing Engine**: Evaluates active thread state and applies VRAM Quota Management logic.
...
- **Context Limit Breaches**: If VRAM Quota Management payload exceeds max token limits, the Semantic Distillation worker is triggered to summarize context.

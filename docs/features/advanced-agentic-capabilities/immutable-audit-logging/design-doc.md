# Design Doc: Immutable Audit Logging
...
The Immutable Audit Logging capability is injected directly into the active LangGraph execution thread.
...
1. **Core Processing Engine**: Evaluates active thread state and applies Immutable Audit Logging logic.
...
- **Context Limit Breaches**: If Immutable Audit Logging payload exceeds max token limits, the Semantic Distillation worker is triggered to summarize context.

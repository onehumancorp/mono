# Design Doc: Fallback Strategy Generation
...
The Fallback Strategy Generation capability is injected directly into the active LangGraph execution thread.
...
1. **Core Processing Engine**: Evaluates active thread state and applies Fallback Strategy Generation logic.
...
- **Context Limit Breaches**: If Fallback Strategy Generation payload exceeds max token limits, the Semantic Distillation worker is triggered to summarize context.

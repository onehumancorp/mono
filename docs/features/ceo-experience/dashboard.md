...
### 3.1 Architecture (Flutter + Flutter Web)
The frontend is a Flutter-based SPA that leverages:
- **State Management**: `Riverpod` for lightweight, reactive Hub state sync.
...
- **Server-Side Rendering (Flutter/Dart)**: Rejected because the dashboard is a high-interactivity real-time tool; a pure SPA with a Go backend is more responsive for WebSocket-heavy workloads.

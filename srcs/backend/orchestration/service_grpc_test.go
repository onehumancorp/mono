...
func (m *MockStreamServer) Send(msg *commonpb.Message) error {
...
func TestStreamMessagesViaGRPC(t *testing.T) {
	...
	stream := &MockStreamServer{ctx: ctx, messages: make([]*commonpb.Message, 0)}
	...
	err := srv.StreamMessages(&pb.HubStreamMessagesRequest{AgentId: "a2"}, stream)
...
func TestReasonViaMinimaxEmptyKey(t *testing.T) {
	...
	req := &pb.HubReasonRequest{Prompt: "test prompt"}

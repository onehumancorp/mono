...
func (m *benchStreamMessagesServer) Send(msg *commonpb.Message) error {
	select {
	case m.ch <- msg:
	default:
	}
...
func BenchmarkStreamLatency(b *testing.B) {
	...
	stream := &benchStreamMessagesServer{
		ctx: ctx,
		ch:  make(chan *commonpb.Message, 100),
	}

	go func() {
		_ = srv.StreamMessages(&pb.HubStreamMessagesRequest{AgentId: "agent2"}, stream)
	}()

...
func TestPublishRoutesMessagesAndMeetingTranscript(t *testing.T) {
	...
	hub.OpenMeeting("kickoff", "org-1", "Kickoff Meeting", []string{"pm-1", "swe-1"})
...
func TestOpenMeetingMarksParticipantsInMeeting(t *testing.T) {
	...
	meeting := hub.OpenMeeting("m1", "org-1", "Meeting 1", []string{"a", "b"})
...
func TestMeetingsReturnsSnapshot(t *testing.T) {
	...
	hub.RegisterAgent(Agent{ID: "a", Name: "A", Role: "PM", OrganizationID: "org-1"})
	hub.RegisterAgent(Agent{ID: "b", Name: "B", Role: "SWE", OrganizationID: "org-1"})
	hub.OpenMeeting("kickoff", "org-1", "Kickoff", []string{"a", "b"})
...
func TestFireAgentRemovesFromHubAndInbox(t *testing.T) {
	...
	hub.RegisterAgent(Agent{ID: "a", Name: "A", Role: "PM", OrganizationID: "org-1"})
	hub.RegisterAgent(Agent{ID: "b", Name: "B", Role: "SWE", OrganizationID: "org-1"})
	hub.OpenMeeting("m1", "org-1", "Meeting 1", []string{"a", "b"})
...
func TestOpenMeetingWithAgendaPreservesAgendaField(t *testing.T) {
	...
	meeting := hub.OpenMeetingWithAgenda("sprint-kickoff", "org-1", "Sprint Kickoff", "Plan Q2 features and assign owners", []string{"pm", "swe"})
...
func (m *mockStreamMessagesServer) Send(msg *commonpb.Message) error {
...
func TestHubServiceServer_StreamMessages(t *testing.T) {
	...
	req := &pb.HubStreamMessagesRequest{
		AgentId: "receiver",
	}
...
func TestHubServiceServer_RegisterAgent(t *testing.T) {
	...
	req := &pb.HubRegisterAgentRequest{
		Agent: &pb.HubAgent{
			Id:             "grpc-agent-1",
			Name:           "GRPC Agent",
			Role:           "TEST_ROLE",
...
func TestHub_Publish_UnbufferedChannel(t *testing.T) {
	...
	meeting := hub.OpenMeeting("m1", "O1", "M1", []string{"receiver"})
	if meeting.ID == "" {
	...
	defer cancel2()
	meeting2 := hub.OpenMeeting("m2", "O1", "M2", []string{"receiver2"})
	if meeting2.ID == "" {
		t.Fatalf("failed to open meeting")
	}
	...
	defer cancel3()
	meeting3 := hub.OpenMeeting("m3", "O1", "M3", []string{"receiver3"})
	if meeting3.ID == "" {
		t.Fatalf("failed to open meeting")
	}
...
func TestHubServiceServer_Publish(t *testing.T) {
	...
	tests := []struct {
		name          string
		req           *pb.HubPublishMessageRequest
		expectSuccess bool
		expectErrCode codes.Code
	}{
		{

...
test("CUJ 1: frontend dashboard loads seeded organization command center", async ({
  page,
}) => {
  await page.goto("/");
  await expect(
    page.getByRole("heading", { name: "One Human Corp Dashboard" }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("heading", { name: "Active Meetings" }),
  ).toBeVisible();

  await saveShot(page, "cuj-01-frontend-dashboard");
});

test("CUJ 2: sending message updates UI and backend transcript", async ({
  page,
  request,
}) => {
  await page.goto("/");
  ...
  await expect(
    page.getByRole("heading", { name: "Virtual War Room" }),
  ).toBeVisible();

  const message = `Playwright seeded message ${Date.now()}`;
  await page
    .getByPlaceholder("Inject direction or approve actions as CEO...")
    .fill(message);
  await page.getByRole("button", { name: "Send" }).click();

  await expect(page.getByText(message)).toBeVisible();

  const meetingsResponse = await request.get(
    "http://127.0.0.1:8080/api/meetings",
  );
...
test("CUJ 3: backend /app route remains reachable for bundled frontend", async ({
  page,
}) => {
  await page.goto("http://127.0.0.1:8080/app");
  await expect(
    page.getByRole("heading", { name: "React Frontend Route" }),
  ).toBeVisible();

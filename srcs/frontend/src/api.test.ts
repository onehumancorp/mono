...
describe("api", () => {
    ...
    const fetchMock = vi.fn(async (input: string) => {
      ...
      if (input === "/api/meetings") {
        return {
          ok: true,
          status: 200,
          json: async () => [{ id: "kickoff", transcript: null }],
        };
      }
      if (input === "/api/costs") {
        return {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async () => ({
        ...
        json: async () => ({
          ...
          totalCostUSD: 1.2,
          agents: [
            { agentID: "pm-1", model: "gpt-4o", tokenUsed: 3, costUSD: 1.2 },
          ],
        }),
      })),
    );
  ...
  it("throws for failed GET request", async () => {
    ...
    await expect(fetchOrganization()).rejects.toThrow(
      "Request failed for /api/org: 500",
    );
  });
...
describe("api – new endpoints", () => {
  ...
  it("fetchDomains returns domain list", async () => {
    const domains = [
      { id: "software_company", name: "Software Company", description: "SaaS" },
    ];
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => ({ ok: true, status: 200, json: async () => domains })),
    );
...
describe("integration api", () => {
  ...
  it("connects an integration", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => ({ ok: true, status: 200, json: async () => updated })),
    );
    await expect(
      connectIntegration("slack", { baseUrl: "https://hooks.slack.com/test" }),
  ...
  it("fetches chat messages", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => ({ ok: true, status: 200, json: async () => msgs })),
    );
  ...
  it("sends a chat message", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => ({ ok: true, status: 200, json: async () => msg })),
    );
  ...
  it("fetches pull requests", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => ({ ok: true, status: 200, json: async () => prs })),
    );
  ...
  it("creates a pull request", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => ({ ok: true, status: 200, json: async () => pr })),
    );
  ...
  it("fetches issues", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => ({ ok: true, status: 200, json: async () => issues })),
    );
  ...
  it("creates an issue", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => ({ ok: true, status: 200, json: async () => issue })),
    );
  ...
  it("updates issue status", async () => {
    ...
    await expect(updateIssueStatus("issue-1", "in_progress")).resolves.toEqual(
      issue,
    );
  });
...
describe("handoff api", () => {
  ...
  it("createHandoff posts and returns handoff package", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => ({ ok: true, status: 200, json: async () => handoff })),
    );
    const result = await createHandoff({
      fromAgentId: "swe-1",
      intent: "Need design review",
      failedAttempts: 2,
...
describe("skill pack api", () => {
  ...
  it("fetchSkillPacks returns list", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => ({ ok: true, status: 200, json: async () => packs })),
    );
  ...
  it("importSkillPack posts and returns imported pack", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => ({ ok: true, status: 200, json: async () => pack })),
    );
...
describe("snapshot api", () => {
  ...
  it("fetchSnapshots returns list", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => ({ ok: true, status: 200, json: async () => snaps })),
    );
  ...
  it("createSnapshot posts label and returns snapshot", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => ({ ok: true, status: 200, json: async () => snap })),
    );
...
describe("marketplace api", () => {
  ...
  it("fetchMarketplace returns items", async () => {
    const items = [
      { id: "item-1", name: "TikTok Virality Expert", type: "agent" },
    ];
    vi.stubGlobal(
      "fetch",
...
describe("analytics api", () => {
  ...
  it("fetchAnalytics returns analytics summary", async () => {
    const analytics = {
      humanAgentRatio: 0.05,
      totalAgents: 20,
      totalHumans: 1,
      auditFidelityPct: 98.5,
      resumptionLatencyMs: 4200,
      pendingApprovals: 2,
      activeHandoffs: 1,
      tokenVelocity: 1500,
    };
...
describe("user management api", () => {
  ...
  it("createUser posts body and returns created user", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => ({ ok: true, status: 200, json: async () => newUser })),
    );
  ...
  it("fetchRoles returns list of roles", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => ({ ok: true, status: 200, json: async () => roles })),
    );
  ...
  it("createRole posts body and returns created role", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => ({ ok: true, status: 200, json: async () => role })),
    );
...
describe("api – authedGetJSON error paths", () => {
  ...
  it("authedGetJSON non-401 throws Request failed error", async () => {
    ...
    await expect(fetchMe()).rejects.toThrow(
      "Request failed for /api/auth/me: 500",
    );
  });
});
...
describe("api – authedPostJSON error paths", () => {
  ...
  it("authedPostJSON 401 clears token and throws Unauthorized", async () => {
    ...
    await expect(
      createUserAuth({ username: "x", email: "x@x.com", password: "p" }),
  ...
  it("authedPostJSON non-401 with error text throws that text", async () => {
    ...
    await expect(
      createUserAuth2({ username: "x", email: "x@x.com", password: "p" }),
  ...
  it("authedPostJSON non-401 with empty text throws Request failed error", async () => {
    ...
    await expect(
      createUserAuth3({ username: "x", email: "x@x.com", password: "p" }),
    ).rejects.toThrow("Request failed for /api/users: 500");
  });
});
...
describe("api – testChatIntegration, invokeMCPTool, saveSettings", () => {
  ...
  it("testChatIntegration posts and returns success", async () => {
    ...
    await expect(
      testChatIntegration("telegram", { botToken: "tok", chatId: "123" }),
  ...
  it("invokeMCPTool posts and returns result", async () => {
    ...
    await expect(
      invokeMCPTool("git-mcp", "invoke", { repo: "my/repo" }),
  ...
  it("saveSettings posts and returns updated settings", async () => {
    ...
    await expect(saveSettings({ minimaxApiKey: "new-key" })).resolves.toEqual({
      minimaxApiKey: "new-key",
    });
  });
});
...
describe("api – authed calls without token (empty auth header branch)", () => {
  ...
  it("authedPostJSON uses empty Authorization when no token stored", async () => {
    ...
    await expect(
      cu({ username: "x", email: "x@x.com", password: "p" }),

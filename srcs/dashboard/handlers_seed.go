...
// handleDevSeed re-seeds the server with an initial scenario and returns a snapshot.
func (s *Server) handleDevSeed(w http.ResponseWriter, r *http.Request) {
	...
	// ⚡ BOLT: [Memory contamination across integration tests]
	// Explicitly clear in-memory state slices before injecting mock data.
	s.handoffs = []HandoffPackage{}
	s.pipelines = []Pipeline{}
	s.approvals = []ApprovalRequest{}

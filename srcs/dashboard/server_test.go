...
func TestHandleIntegrationConnectBlocksSSRF(t *testing.T) {
	_, server := newTestServer(t)
	defer server.Close()
	...
	for _, badURL := range badURLs {
		body := `{"integrationId":"slack","baseUrl":"` + badURL + `"}`
		resp, err := http.Post(server.URL+"/api/integrations/connect", "application/json", strings.NewReader(body))
		if err != nil {
			t.Fatalf("unexpected error for %s: %v", badURL, err)
		}
		resp.Body.Close()
		if resp.StatusCode != http.StatusBadRequest {
			t.Errorf("expected 400 for URL %q, got %d", badURL, resp.StatusCode)
		}
	}
}
...
func TestHandleB2BHandshakeBlocksSSRF(t *testing.T) {
	_, server := newTestServer(t)
	defer server.Close()
	...
	for _, badURL := range badURLs {
		body, _ := json.Marshal(map[string]any{
			"partnerOrg":     "globex.com",
			"partnerJwksUrl": badURL,
			"allowedRoles":   []string{"SALES_AGENT"},
		})
		resp, err := http.Post(server.URL+"/api/b2b/handshake", "application/json", bytes.NewReader(body))
		if err != nil {
			t.Fatalf("unexpected error for %s: %v", badURL, err)
		}
		resp.Body.Close()

...
func TestHandleHireAgentAddsToHub(t *testing.T) {
	...
	for _, a := range snapshot.Agents {
		if a.Name == "New SWE" && a.Role == "SOFTWARE_ENGINEER" && a.Model != "" {
			found = true
		}
	}
	if !found {
...
func TestHandleModelsReturnsCatalog(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("GET /api/models returned error: %v", err)
	}
	defer resp.Body.Close()
	...
	if len(models) == 0 {
		t.Fatalf("expected at least one supported model")
	}
	if models[0].ID == "" {
		t.Fatalf("expected model id to be populated")
	}
}

func TestHandleA2AMessagePublishesToMeeting(t *testing.T) {
	app, server := newTestServer(t)
	defer server.Close()

	body := bytes.NewBufferString(`{"fromAgentId":"pm-1","toAgentId":"swe-1","conversationId":"kickoff","intent":"handoff","payload":"Please take over the implementation."}`)
	req, err := http.NewRequest(http.MethodPost, server.URL+"/api/a2a/messages", body)
	if err != nil {
		t.Fatalf("new request: %v", err)
	}
	req.Header.Set("Content-Type", "application/json")
	...
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		t.Fatalf("POST /api/a2a/messages returned error: %v", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("expected 200 from a2a publish, got %d", resp.StatusCode)
	}

	meeting, _ := app.hub.Meeting("kickoff")
	if len(meeting.Transcript) == 0 {
		t.Fatalf("expected kickoff transcript to receive A2A message")
	}
	last := meeting.Transcript[len(meeting.Transcript)-1]
	if last.Protocol != "A2A/1.0" {
		t.Fatalf("expected A2A protocol, got %q", last.Protocol)
	}
}

func TestHandleDelegateTasksCreateAndList(t *testing.T) {
	_, server := newTestServer(t)
	defer server.Close()

	body := bytes.NewBufferString(`{"providerId":"github-copilot","title":"Implement feature","goal":"Ship feature in one shot","systemPrompt":"Use MCP and skills to execute","preferredModel":"gpt-4o","mcpServers":["git-mcp"],"skills":["software-dev"]}`)
	req, err := http.NewRequest(http.MethodPost, server.URL+"/api/delegate/tasks", body)
	if err != nil {
		t.Fatalf("new request: %v", err)
	}
	req.Header.Set("Content-Type", "application/json")
	...
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		t.Fatalf("POST /api/delegate/tasks returned error: %v", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("expected 200 from delegate task create, got %d", resp.StatusCode)
	}

	listResp, err := http.Get(server.URL + "/api/delegate/tasks")
	if err != nil {
		t.Fatalf("GET /api/delegate/tasks returned error: %v", err)
	}
	defer listResp.Body.Close()

	var tasks []DelegateTask
	if err := json.NewDecoder(listResp.Body).Decode(&tasks); err != nil {
		t.Fatalf("decode delegate task list: %v", err)
	}
	if len(tasks) != 1 {
		t.Fatalf("expected 1 delegate task, got %d", len(tasks))
	}
	if tasks[0].ProviderID != "github-copilot" || tasks[0].Status != "queued" {
		t.Fatalf("unexpected delegate task payload: %+v", tasks[0])
	}
}

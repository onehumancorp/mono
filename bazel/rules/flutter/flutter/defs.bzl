...
# Try to use dart pub directly to avoid Flutter's global SDK lock
DART_BIN="$FLUTTER_ROOT/bin/cache/dart-sdk/bin/dart"
if [ ! -f "$DART_BIN" ]; then
    DART_BIN="$FLUTTER_BIN_ABS"
    PUB_ARGS=("pub" "get" "--offline")
else
    PUB_ARGS=("pub" "get" "--offline")
fi

if "$DART_BIN" "${{PUB_ARGS[@]}}" | tee -a "$TEST_LOG" 2>&1; then
    echo "✓ Package config regenerated successfully (offline)" | tee -a "$TEST_LOG"
else
    echo "✗ pub get --offline failed in test runtime" | tee -a "$TEST_LOG"
    popd >/dev/null
    exit 1
fi
popd >/dev/null
